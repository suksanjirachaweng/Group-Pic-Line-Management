...
const TABS = [
  ...
  { slug: "registrants", label: "Registrants", linePrefix: true },
      ...
      <nav className="mb-6 flex gap-1 overflow-x-auto border-b border-gray-200">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {"linePrefix" in tab && tab.linePrefix && (
                <span className="mr-1.5 inline-flex items-center rounded-full bg-green-500 px-1.5 py-0.5 text-[10px] font-semibold text-white">
                  LINE
                </span>
              )}
              {tab.label}
